  ...
  <div class="retrieve">
    <div class="flex-center flex justify-center items-center flex-col">
      <el-steps :space="400" :active="state.active" finish-status="success" process-status="finish" align-center>
        <el-step title="验证邮箱" :icon="Message"> </el-step>
        <el-step title="填写密码" :icon="Lock"> </el-step>
      ...
      <div class="form_wrap">
        <el-form class="retrieve-form" :model="state.loginForm" :rules="state.rules" ref="formRef" size="large">
          <el-form-item prop="email">
            <el-input v-model="state.loginForm.email" :prefix-icon="Message" placeholder="请输入邮箱"> </el-input>
          </el-form-item>
          ...
          <el-form-item prop="checkPass" v-if="state.active == 1">
            <el-input
              type="password"
              v-model="state.loginForm.checkPass"
              autocomplete="off"
        ...
        <div class="tips flex justify-between">
          <yh-button size="large" style="width: 30%" @click="back()" variant="outline">返回 </yh-button>
          <yh-button
            size="large"
            theme="primary"
